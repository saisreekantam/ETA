"""
Trains HybridRiskGNN (graph + PCA-augmented zone features) on the identical
windows/splits/normalization/loss discipline as train.py and train_no_graph.py, so its
result is a clean three-way "graph alone vs PCA alone vs graph+PCA" comparison.

Run: `python -m eval.hai_validation.train_hybrid` (after precompute_pca_features.py).
"""
...
from eval.hai_validation.dataset_hybrid import HaiWindowDatasetHybrid
from eval.hai_validation.dataset import TEST_FILES, TRAIN_FILES, VAL_FILES
from eval.hai_validation.model_hybrid import NODE_TYPES, HybridRiskGNN
...
OUT_DIR = Path(__file__).resolve().parent / "checkpoints_hybrid"
...
def main():
    ...
    train_ds = HaiWindowDatasetHybrid(TRAIN_FILES)
    val_ds = HaiWindowDatasetHybrid(VAL_FILES)
    test_ds = HaiWindowDatasetHybrid(TEST_FILES)
